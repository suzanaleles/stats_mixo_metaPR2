...
# MetaPR2 data + Mixoplankton Database: analysis for V9-total dataset                                   #
...
pc1 <- p + geom_point(data = data_eco_wide2 %>% filter(depth == "surface"), aes(x = long, y = lat, fill = factor(comclust)), size = 4, shape = 21) +
...
ggsave("fig_SOM_map_V9_total.png", pc1, device = "png", width = 6, height = 4, dpi = 300) # 8.66
...
ggsave("fig_SOM_map_by_cluster_V9_total.png", pbyc, device = "png", width = 8, height = 5, dpi = 300) # 8.66
...
som_map_V9_total <- data_eco_wide_env %>% select(long,lat,comclust,temperature,NO3,salinity) %>%
  mutate(dataset = "V9-total")

write.csv(som_map_V9_total, "som_map_V9_total.csv")
...
ggsave("fig_V9_total_env_V9_total.png", pt+ps+pn, device = "png", width = 13, height = 4, dpi = 300) # 8.66
...
ggsave("fig_V9_total_nitrate_hist_V9_total.png", pnhist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_V9_total_temp_hist_V9_total.png", pthist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_V9_total_sal_hist_V9_total.png", pshist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_rare_not_rare_V9_total.png", pc4, device = "png", width = 5, height = 11, dpi = 300) # 8.66
...
df_70 <- data_eco_wide5 %>%
  ...
  filter(cum_abundance <= 0.5) 

write.csv(df_70, "v5_sum_by_genus_averaged_by_station_V9total.csv")
...
ggsave("fig_heatmap_genus_V9total.png", fig4, device = "png", width = 5.5, height = 8, dpi = 300) 
...
rules <- tibble(
  ...
  biome = c("temperate/subtropical", "temperate/subtropical", "subpolar/temperate", "subtropical/tropical", "subtropical/tropical")
)
...
ggsave("fig_heatmap_genus_V9total_by_biome.png", fig4b, device = "png", width = 3.7, height = 7.5, dpi = 300)
...
ggsave("fig_rel_ab_V9_total.png", pc5, device = "png", width = 6.5, height = 4, dpi = 300)
...
write_csv(model_summary, "model_comparison_summary_total-V9_temp.csv")
...
ggsave("fig_gam_temp_total_V9.png", pgamt, device = "png", width = 26, height = 3.5, dpi = 300)

# Do the same now for salinity

# if removing three outliers from V9-total
data_eco_long3 <- data_eco_long3 %>%
  filter(between(salinity, 30, 38))
...
write_csv(model_summary, "model_comparison_summary_total-V9_sal.csv")
...
ggsave("fig_gam_sal_total_V9.png", pgams, device = "png", width = 26, height = 3.5, dpi = 300)
...
write_csv(model_summary, "model_comparison_summary_total-V9_nitrate.csv")
...
ggsave("fig_gam_nitrate_total_V9.png", pgams, device = "png", width = 26, height = 3.5, dpi = 300)
...
data_eco_wide6 <- data_eco_wide6 %>% mutate(dataset = "V9-total")
write.csv(data_eco_wide6, "som_map_V9_total_fig3.csv")
...
df1$envclust <- data_rda$envclust
df1$comclust <- data_rda$comclust

write.csv(rda_env2, "rda_V9_total.csv", row.names = FALSE)
...
df1 <- df1 %>% mutate(dataset = "V9-total")
df3 <- df3 %>% mutate(dataset = "V9-total")
write.csv(df1, "rda_V9_total1.csv")
write.csv(df3, "rda_V9_total2.csv")
...
ggsave("fig3_V9_total.png", fig3, device = "png", width = 10, height = 10, dpi = 600) 
