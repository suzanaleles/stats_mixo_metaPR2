...
d2 <- d2 %>% filter(d2$gene_region == "V4")
...
write_csv(unique_combinations_asv_species_genus, "all_v5_asv_species_genus_V4region.csv")

# Exporting merged data for V4 and euphotic (without seaweeds, i.e. "excluded")
write_csv(d2, "all_v5_all_merged_data_V4_euphotic.csv")
...
ggsave("fig_SOM_map_V4_total.png", pc1, device = "png", width = 6, height = 4, dpi = 300) # 8.66
...
ggsave("fig_SOM_map_by_cluster_V4_total.png", pbyc, device = "png", width = 8, height = 5, dpi = 300) # 8.66

# checking if there are biases by specific datasets by community cluster
data_check <- data.frame(station = d2$file_code,
                        dataset = d2$dataset_name)

length(unique(data_check$dataset))

data_check <- data_check %>%
  distinct(station, dataset, .keep_all = TRUE)

data_eco_wide2p <- data_eco_wide2

data_eco_wide2p <- data_eco_wide2p %>%
  left_join(data_check %>% select(station, dataset), by = "station")

length(unique(data_eco_wide2p$dataset))

pproject1 <- p + geom_point(data = data_eco_wide2p %>% filter(comclust == "1"), aes(x = long, y = lat), size = 4, shape = 21, fill = col_com[1]) +
  facet_wrap(~ dataset) +
  theme(legend.key = element_blank()) +
  theme(legend.title = element_text(size = 15, colour = "black"),
        legend.position = "top",
...
ggsave("fig_clust1_project_V4_total.png", pproject1, device = "png", width = 12, height = 8, dpi = 300) # 8.66

pproject2 <- p + geom_point(data = data_eco_wide2p %>% filter(comclust == "2"), aes(x = long, y = lat), size = 4, shape = 21, fill = col_com[2]) +
  facet_wrap(~ dataset) +
  theme(legend.key = element_blank()) +
  theme(legend.title = element_text(size = 15, colour = "black"),
        legend.position = "top",
...
ggsave("fig_clust2_project_V4_total.png", pproject2, device = "png", width = 12, height = 8, dpi = 300) # 8.66

pproject3 <- p + geom_point(data = data_eco_wide2p %>% filter(comclust == "3"), aes(x = long, y = lat), size = 4, shape = 21, fill = col_com[3]) +
  facet_wrap(~ dataset) +
  theme(legend.key = element_blank()) +
  theme(legend.title = element_text(size = 15, colour = "black"),
        legend.position = "top",
...
ggsave("fig_clust3_project_V4_total.png", pproject3, device = "png", width = 12, height = 8, dpi = 300) # 8.66

pproject4 <- p + geom_point(data = data_eco_wide2p %>% filter(comclust == "4"), aes(x = long, y = lat), size = 4, shape = 21, fill = col_com[4]) +
  facet_wrap(~ dataset) +
  theme(legend.key = element_blank()) +
  theme(legend.title = element_text(size = 15, colour = "black"),
        legend.position = "top",
...
ggsave("fig_clust4_project_V4_total.png", pproject4, device = "png", width = 12, height = 8, dpi = 300) # 8.66

pproject5 <- p + geom_point(data = data_eco_wide2p %>% filter(comclust == "5"), aes(x = long, y = lat), size = 4, shape = 21, fill = col_com[5]) +
  facet_wrap(~ dataset) +
  theme(legend.key = element_blank()) +
  theme(legend.title = element_text(size = 15, colour = "black"),
        legend.position = "top",
...
ggsave("fig_clust5_project_V4_total.png", pproject5, device = "png", width = 12, height = 8, dpi = 300) # 8.66
...
nrow(data_eco_wide_env)

data_eco_wide_env %>%
  summarise(n_below_20 = sum(salinity < 20, na.rm = TRUE))

data_eco_wide_env <- data_eco_wide_env %>%
  filter(between(salinity, 20, 40))
nrow(data_eco_wide_env)
...
som_map_V4_total <- data_eco_wide_env %>% select(long,lat,comclust,temperature,NO3,salinity) %>%
  mutate(dataset = "V4-total")

write.csv(som_map_V4_total, "som_map_V4_total.csv")
...
ggsave("fig_V4_total_env_V4_total.png", pt+ps+pn, device = "png", width = 13, height = 4, dpi = 300) # 8.66
...
ggsave("fig_V4_total_nitrate_hist_V4_total.png", pnhist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_V4_total_temp_hist_V4_total.png", pthist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_V4_total_sal_hist_V4_total.png", pshist, device = "png", width = 10, height = 6, dpi = 300) # 8.66
...
ggsave("fig_rare_not_rare_V4_total.png", pc4, device = "png", width = 5, height = 11, dpi = 300) # 8.66
...
df_70 <- data_eco_wide5 %>%
  ...
  filter(cum_abundance <= 0.6) 

write.csv(df_70, "v5_sum_by_genus_averaged_by_station_V4total.csv")
...
# order "genus" by "type"
df_heatmap$type <- factor(df_heatmap$type, levels = c("CM", "eSNCM", "not assessed", "phytoplankton", "protozooplankton", "parasite")[6:1])
...
colors_type <- c("#B2182B", "#D6604D", "lightgray", "#92C5DE", "#4393C3", "#2166AC")
colors_type2 <- c("#2166AC", "#4393C3", "#92C5DE", "lightgray","#D6604D","#B2182B")
...
ggsave("fig_heatmap_genus_V4total.png", fig4, device = "png", width = 5.5, height = 7.8, dpi = 300) 
...
df_heatmap$biome <- factor(df_heatmap$biome, levels = c("polar", "subpolar/temperate", "temperate/subtropical", "subtropical/tropical"),
                           labels = c("P","Sp-Te", "Te-St", "St-Tr"))
...
fig4b <- ggplot() +
  ...
  coord_cartesian(expand = FALSE, xlim = c(0.6, 4.5), clip = "off") +
  theme_bw() +
  theme(legend.position = "top",
        axis.text = element_text(size = 10, colour = "black"),
        axis.text.y = element_markdown(hjust = 1, margin = margin(r = 10)),
...
ggsave("fig_heatmap_genus_V4total_by_biome.png", fig4b, device = "png", width = 3.7, height = 7.5, dpi = 300)
...
ggsave("fig_rel_ab_V4_total.png", pc5, device = "png", width = 6.5, height = 4, dpi = 300)
...
min(data_eco_long3$salinity, na.rm = "TRUE")
...
write_csv(model_summary, "model_comparison_summary_total-V4_temp.csv")
...
data_eco_long3 <- data_eco_long3 %>%
  filter(!(type == "GNCM" & sum > 0.02))

data_eco_long3 <- data_eco_long3 %>%
  filter(!(type == "pSNCM" & sum > 0.1))
...
ggsave("fig_gam_temp_total_V4.png", pgamt, device = "png", width = 26, height = 3.5, dpi = 300)
...
model_summary <- model_comparison %>%
  left_join(gam_stats, by = "type") %>%
  #left_join(glm_stats, by = "type") %>%
  #left_join(lm_stats, by = "type") %>%
  select(type, best_model,gam_edf, gam_F, gam_p, dev_expl, lm_aic, glm_aic, gam_aic)

write_csv(model_summary, "model_comparison_summary_total-V4_sal.csv")
...
ggsave("fig_gam_sal_total_V4.png", pgams, device = "png", width = 26, height = 3.5, dpi = 300)
...
min(data_eco_long3$NO3, na.rm = "TRUE")
...
write_csv(model_summary, "model_comparison_summary_total-V4_nitrate.csv")
...
ggsave("fig_gam_nitrate_total_V4.png", pgams, device = "png", width = 26, height = 3.5, dpi = 300)
...
data_eco_wide_test <- md %>% filter(sizefraction == "total") %>%
  group_by(type, station, comclust) %>%
  summarize(avg = sum(relab, na.rm = TRUE))
...
data_eco_wide6 <- data_eco_wide6 %>% mutate(dataset = "V4-total")
write.csv(data_eco_wide6, "som_map_V4_total_fig3.csv")
...
# Filter only fraction_name = total
...
min(data_eco_wide$salinity, na.rm = "TRUE")
max(data_eco_wide$salinity, na.rm = "TRUE")

data_eco_wide <- data_eco_wide %>%
  filter(between(salinity, 20, 40))
...
#rda_env <- rda_env2[,c(2:9)]
#rda_env <- rda_env2[,c(2,7:9,13:15)]
nrow(rda_env)
...
#plot(dbRDA, scaling = 3)
#dbRDA_stats <- anova(dbRDA, step=1000, perm.max = 1000)
...
write.csv(rda_env2, "rda_V4_total.csv", row.names = FALSE)
...
df1 <- df1 %>% mutate(dataset = "V4-total")
df3 <- df3 %>% mutate(dataset = "V4-total")
write.csv(df1, "rda_V4_total1.csv")
write.csv(df3, "rda_V4_total2.csv")
...
ggsave("fig3_V4_total.png", fig3, device = "png", width = 10, height = 10, dpi = 600) 
